...
public class IndexNode {
  ...
  /**
   * Maximum number of blocks in a file. If indirect, doubleIndirect, or tripleIndirect blocks are
   * implemented, this number will need to be increased.
   */
  public static final int MAX_FILE_BLOCKS = MAX_DIRECT_BLOCKS;

  /** Mode for this index node. This includes file type and file protection information. */
  ...
  /**
   * Array of direct blocks containing the block addresses for the first MAX_DIRECT_BLOCKS blocks of
   * the file. Note that each element in the array is stored as a 3-byte number on disk.
   */
   ...
   * @return the address of the block, a number between zero and one less than the number of blocks
   *     in the file system
   * @exception java.lang.Exception if the block number is invalid
   */
  public int getBlockAddress(int block) throws Exception {
    if (block >= 0 && block < MAX_DIRECT_BLOCKS) return (directBlocks[block]);
    else throw new Exception("invalid block address " + block);
  }
   ...
   *
   * @param block the sequential block number
   * @param address the address of the block, a number between zero and one less than the number of
   *     blocks in the file system
   * @exception java.lang.Exception if the block number is invalid
   */
  public void setBlockAddress(int block, int address) throws Exception {
    if (block >= 0 && block < MAX_DIRECT_BLOCKS) directBlocks[block] = address;
    else throw new Exception("invalid block address " + block);
  }
  ...
  /**
   * Writes the contents of an index node to a byte array. This is used to copy the bytes which
   * correspond to the disk image of the index node onto a block buffer so that they may be written
   * to the file system.
   *
   * @param buffer the buffer to which bytes should be written
   * @param offset the offset from the beginning of the buffer at which bytes should be written
   */
  ...
  /**
   * Reads the contents of an index node from a byte array. This is used to copy the bytes which
   * correspond to the disk image of the index node from a block buffer that has been read from the
   * file system.
   *
   * @param buffer the buffer from which bytes should be read
   * @param offset the offset from the beginning of the buffer at which bytes should be read
   */
  ...
  public void copy(IndexNode indexNode) {
    ...
    indexNode.gid = gid;
    indexNode.size = size;
    for (int i = 0; i < MAX_DIRECT_BLOCKS; i++) indexNode.directBlocks[i] = directBlocks[i];
