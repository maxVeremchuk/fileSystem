...
public class DirectoryEntry {
  ...
  /**
   * Constructs a DirectoryEntry for the given inode and name. Note that the name is stored
   * internally as a byte[], not as a string.
   *
  ...
  public void setName(String newName) {
    for (int i = 0; i < MAX_FILENAME_LENGTH && i < newName.length(); i++)
      if (i < newName.length()) d_name[i] = (byte) newName.charAt(i);
      else d_name[i] = (byte) 0;
  }
   ...
   * @param offset the offset from the beginning of the buffer to which the directory entry should
   *     be written
   */
  public void write(byte[] buffer, int offset) {
    buffer[offset] = (byte) (d_ino >>> 8);
    buffer[offset + 1] = (byte) d_ino;
    for (int i = 0; i < d_name.length; i++) buffer[offset + 2 + i] = d_name[i];
  }
   ...
   * @param offset the offset from the beginning of the buffer from which the directory entry should
   *     be read
   */
  public void read(byte[] buffer, int offset) {
    ...
    d_ino = (short) (hi << 8 | lo);
    for (int i = 0; i < d_name.length; i++) d_name[i] = buffer[offset + 2 + i];
  }
