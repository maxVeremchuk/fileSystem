...
public class FileSystem {
   ...
   * @exception java.io.IOException if any IOExceptions are thrown during the open.
   */
   ...
   *
   * @exception java.io.IOException if the open or read causes IOException to be thrown
   */
   ...
   *
   * @exception java.io.IOException if the closing the backing file causes any IOException to be
   *     thrown
   */
   ...
   * @exception java.io.IOException if there are any exceptions during the read from the underlying
   *     "file system" file.
   */
   ...
   * @exception java.io.IOException if there are any exceptions during the write to the underlying
   *     "file system" file.
   */
   ...
   * @exception java.io.IOException if any exception occurs during an operation on the underlying
   *     "file system" file.
   */
   ...
   *
   * @return the data block number which was allocated; -1 if no blocks are available
   * @exception java.io.IOException if any exception occurs during an operation on the underlying
   *     "file system" file.
   */
  public int allocateBlock() throws IOException {
    ...
    while (true) {
      ...
      if (currentFreeListBitNumber >= (blockCount - dataBlockOffset)) currentFreeListBitNumber = 0;
      if (!allocated) {
  ...
  /**
   * Loads the block containing the specified data block bit into the free list block buffer. This
  ...
  /**
   * The number of the index node block which is currently loaded into indexBlockBytes. If no block
   * is loaded, this contains the value "-1".
   */
  private short currentIndexNodeBlock = -1;

  /**
   * The byte buffer used for reading and writing index node blocks. You can think of this as a
   * one-block cache.
   */
   ...
   *
   * @return the inode number for the next available index node; -1 if there are no index nodes
   *     available.
   * @exception java.io.IOException if there is an exception during an operation on the underlying
   *     "file system" file.
   */
  public short allocateIndexNode() throws IOException {
    ...
    while (true) {
      ...
      if (currentIndexNodeNumber
          >= ((dataBlockOffset - inodeBlockOffset) * (blockSize / IndexNode.INDEX_NODE_SIZE)))
   ...
   * @execption java.io.IOException if any exception occurs in an underlying operation on the "file
   *     system" file.
   */
  ...
  /**
   * Loads the block containing the specified index node into the index node block buffer. This is a
   * convenience method.
   *
